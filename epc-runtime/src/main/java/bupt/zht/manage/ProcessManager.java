...
import bupt.zht.pubsub.ExecuteFunction;
...
public class ProcessManager {
    ...
    public void parseSchema(String schemaFile){
        ...
        if("start".equals(message)){
            ProcessInstance processInstance = createProcessInstance(modelId,instanceId);
    ...
    public ProcessInstance createProcessInstance(String processModelID,String processInstanceID){
        ...
        EpcCompiler instanceEpcCompiler = null;
        try {
            instanceEpcCompiler = (EpcCompiler) modelEpcCompiler.deepClone();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void judgeRootEvent(ProcessInstance processInstance,String theme,String message){
        ...
        if (rootObject instanceof Event) {
            ...
            ExecuteFunction.executeFunction(function, message);
            return;
        }
        // 触发已有的流程实例，更新流程实例状态
