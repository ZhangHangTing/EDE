...
public abstract class JacobObject implements Serializable {
     ...
     * java代码中的instance（new F（x，y，z））等价于；流程运算中的标签F（x，y，z）
    ...
    /**
     * 下面的多个重载方法object(x, ChannelListener)等价于流程代数中的x ? ChannelListener
     * Object; the Java code "object(x, ChannelListener)" is equivalent to
     * <code>x ? ChannelListener</code> in the process .
     * 
