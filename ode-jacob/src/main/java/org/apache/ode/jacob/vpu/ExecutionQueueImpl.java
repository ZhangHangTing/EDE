...
public class ExecutionQueueImpl implements ExecutionQueue {
     ...
     */
    //缓存set存储入队的Continuation对象，这些反应器是被缓存的，并不是直接放到DAO层
    // Continuation入队之后，在下一次的轮询中就马上出队
    ...
    //HashSet是无序的状态保存的（就是用HashMap实现的一个容器，并不保证排序）,随机出队一个Continuation，
...
//        XQXMLOutputStream sos = createObjectOutputStream(new OutputStreamWriter(oos));
    ...
    //Externalizable接口继承了Serializable接口，同时增加了两个方法writeExternal和readExternal
